...
- 流程：启动器 -> 等待启动按钮可用（灰转蓝）-> 点击启动按钮中心点 -> 网页登录 -> 等待客户端 -> 选频道 -> 选角色 -> 进入游戏 -> 等待 30 秒 -> 退出
...
- 启动器路径建议放在 .env（LAUNCHER__EXE_PATH）以适配不同设备
- 日志按日期分割保存
- 代码风格遵循 PEP 8
- 调度：启动时生成当天两次执行时间，若已过则顺延
...
### M5：PyQt6 前端（0.5 天）
- 设置页：原始 YAML 编辑器 + 调度设置（两次时间/最小间隔/随机范围）
- 执行页：账号级 + 当前步骤
...
- 函数扁平化，避免不必要的类与层级
- 调度默认随机窗口，允许切换固定时间
- 启动时生成当天两次执行时间，已过则立即补跑一次；防重入锁 `logs/run.lock`
