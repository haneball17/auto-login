...
- runner.py：账号轮换、流程推进、失败重试，支持 stop.flag 优雅停止；账号断点写入 `logs/state.json`

- scheduler.py：APScheduler 常驻调度、互斥锁、补跑策略
...
- 角色界面标题/角色模板超时未匹配时，先扩大 ROI（上下左右扩 200%）再重试
