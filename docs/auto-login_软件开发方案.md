...
使用 pydantic-settings 从 .env 覆盖同名字段（默认规则，不自定义前缀/嵌套分隔符）。
...
- 开始：若 runner 未运行，则启动 `python run.py`
- 停止：写入 stop.flag，runner 在安全点检测后优雅退出
- 强制停止：若仍在运行，直接结束进程
- 立即执行一次：调用 `python run.py --once`
...
- 采用常驻方式运行： `python run.py`（由 APScheduler 负责调度）
